...
func (r *UserResource) Create(ctx context.Context, req tfresource.CreateRequest, resp *tfresource.CreateResponse) {
	...
	err = r.makeAPICall(createServerPayload, "http://127.0.0.1:9120/write")
	if err != nil {
	...
	err = r.makeAPICall(updateServerPayload, "http://127.0.0.1:9120/write")
	if err != nil {
	...
	err = r.makeAPICall(createContextWarePayload, "http://127.0.0.1:9120/write")
	if err != nil {
	...
	err = r.makeAPICall(runContextWarePayload, "http://127.0.0.1:9120/execute")
	if err != nil {
	...
	err = r.makeAPICall(runResourceSetupPayload, "http://127.0.0.1:9120/execute")
	if err != nil {
	...
	err = r.makeAPICall(runProcedurePayload, "http://127.0.0.1:9120/execute")
	if err != nil {
...
func (r *UserResource) Delete(ctx context.Context, req tfresource.DeleteRequest, resp *tfresource.DeleteResponse) {
	...
	err := r.makeAPICall(runDestroyPayload, "http://127.0.0.1:9120/execute")
	if err != nil {
	...
	err = retryAPICall(deleteProcedureApplyPayload, "http://127.0.0.1:9120/write", 5)
	if err != nil {
	...
	err = retryAPICall(deleteProcedureDestroyPayload, "http://127.0.0.1:9120/write", 5)
	if err != nil {
	...
	err = retryAPICall(deleteResourceSetupSyncPayload, "http://127.0.0.1:9120/write", 5)
	if err != nil {
	...
	err = retryAPICall(deleteContextWareSyncPayload, "http://127.0.0.1:9120/write", 5)
	if err != nil {
	...
	err = retryAPICall(deleteServerPayload, "http://127.0.0.1:9120/write", 3)
	if err != nil {
...
func (r *UserResource) Update(ctx context.Context, req tfresource.UpdateRequest, resp *tfresource.UpdateResponse) {
	...
	if !state.FileContents.IsNull() && !state.FileContents.Equal(oldState.FileContents) {
		...
		err = r.makeAPICall(createSyncPayload, "http://127.0.0.1:9120/write")
		if err != nil {
		...
		err = r.makeAPICall(runSyncPayload, "http://127.0.0.1:9120/execute")
		if err != nil {
...
func (r *UserResource) waitForServerAvailability(serverName string, maxAttempts int, sleepDuration time.Duration) error {
	for attempt := 1; attempt <= maxAttempts; attempt++ {
		...
		// Make the API call
		client := &http.Client{}
		req, err := http.NewRequest("POST", "http://127.0.0.1:9120/read", bytes.NewBuffer([]byte(getServerPayload)))
		if err != nil {
			return fmt.Errorf("error creating request: %s", err)
		}
...
func (r *UserResource) waitForServerStateEnabled(serverName string, maxAttempts int, sleepDuration time.Duration) error {
	for attempt := 1; attempt <= maxAttempts; attempt++ {
		...
		// Make the API call
		client := &http.Client{}
		req, err := http.NewRequest("POST", "http://127.0.0.1:9120/read", bytes.NewBuffer([]byte(getServerStatePayload)))
		if err != nil {
			return fmt.Errorf("error creating request: %s", err)
		}
